...
共 72 个用例，覆盖：按键名解析与校验、连点/定时任务的启停与节拍、前台窗口匹配（包含/精确/正则/空关键词/非法正则/大小写）、窗口锁定下不下发按键、主题生成与切换持久化、配置文件路径优先级（环境变量 / 便携模式 / 用户目录）、配置往返与旧格式兼容、按键录入器（组合键、长按去重、Esc 取消、修饰键忽略）、界面构建与热键行为。测试使用 Qt 的 `offscreen` 平台，无需显示器，也不会读写真实的用户配置目录。
...
```
keyclicker.py              主程序（单文件，约 2000 行）
KeyClicker.spec            PyInstaller 打包配置
tests/test_keyclicker.py   单元测试（72 个用例，标准库 unittest）
